...
+	Flag	Always show the sign of a signed number	%+d, 42 → +42
 	Flag	Put a space before a positive signed number if no sign is displayed	% d, 42 → _42
...
Precision has different meanings depending on the conversion.

Conversion	Meaning of precision	Example
...
For strings, precision can reduce the amount of the string that is printed.

For integers, precision never removes significant digits. If necessary, zeroes are added to reach the requested precision.

Priority, Conflicts and Exceptions

Situation	Rule	Example
- + 0	- wins; 0 is ignored	%-05d, 42 → 42___
Integer precision + 0	Precision wins; the 0 flag is ignored	%08.5d, 42 → ___00042
...
Width smaller than output	Width never truncates the output	%3d, 12345 → 12345
String shorter than precision	Do not add characters	%.5s, "Hi" → Hi
String longer than precision	Print only up to the precision	%.3s, "Hello" → Hel
...
Integer value 0 with precision 0	Print no digits	%.0d, 0 → empty
Negative number + 0	- appears before zero padding	%05d, -42 → -0042
+ + 0	+ appears before zero padding	%+05d, 42 → +0042
# + 0	0x / 0X appears before zero padding	%#08x, 42 → 0x00002a
- + width	Padding goes on the right	%-5d, 42 → 42___
Width without -	Padding goes on the left	%5d, 42 → ___42

Main Priority Rules

The three main conflicts to remember are:
...
* 8 requires the entire field to occupy at least 8 characters
* The remaining 3 characters are padding

Result:

___00042

Width is always a minimum field width. It does not truncate output that is longer than the requested width.
